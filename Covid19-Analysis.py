...
b=[] 
Death_growing_rate=[] 
df=pd.read_csv('time_series_covid_19_deaths.csv',delimiter=',') 
country_deaths=[list(row) for row in df.values] 
for i in range(0,245): 
    t1=0 for j in range(1,76): 
        t1=t1+country_deaths[i][j] 
        a.append(t1) 
        #code death in different countries as a list

cr=pd.read_csv('time_series_covid_19_recovered.csv',delimiter=',') 
...
#converting the whole csv data into a lists in list df=pd.read_csv('high_risk_countries_deaths.csv',delimiter=',') high_risk_countries_deaths=[list(row) for row in df.values]

df=pd.read_csv('high_risk_countries_recovered.csv',delimiter=',') 
high_risk_countries_recovered=[list(row) for row in df.values]

#code recovered in different cases as a list print("1.Death_growing_rates of all countries \n2.death_growing_rate of countries more than 4% \n3.Death growing rate countries with 100% \n4.high_risk_countries_for which indian students go for study,internship etc.,") m=int(input())
